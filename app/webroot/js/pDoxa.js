...
function iniciar() {
    ...
    $("#mainmenu a.ayuda").click(function(){
        $(this).target = "_blank";
        window.open($(this).prop('href'),"ayuda", "height=400,width=600");
        return false;
    });
...
function reloadAjax(etiqueta) {
    ...
    $.ajax({
        ...
        complete: function () {
            refrescarBotonesAcciones(etiqueta);
            cargador(null);
        }
    });
}
...
function refrescarContenido(){
    ...
    var tabsAulas = $("#content div#tabsContenido").tabs({
        ...
        beforeLoad: function (event, ui) {
			//ui.panel.html('<img src="' + globalUrl + 'img/loader.gif" /> Cargando...');
			cargador("Cargando Datos...");

            //alert($( "div#tabsContenido" ).tabs( "option", "active"));
        },
        load: function () {
            refrescarBotonesAcciones('#content');
            toogleDatosImpresion();
            refrescarCuadroAula(); 
...
function refrescarPaginacion() {
    ...
    refrescarBotonesAcciones('#paginacion');
}
...
//function refrescarBotonesAccionesContent(){
    ...
    $(etiqueta+' .acciones a').button().click(function () {
        ...
        if ($(this).attr('id') == 'alerta') {
            ...
            alert('Alerta de Seguridad\nPara realizar esta acción primero asegure la información');
            if (!confirm('¿Esta seguro que desea realizar esta acción?')) {
                $('.acciones a').button('enable');
                $('.acciones a.inactivo').button('disable');
                return false;
    ...
    $(etiqueta+' .boton a, #content div.boton').button();
    
    $(etiqueta+' .popup a,'+etiqueta+' .vistaImpresion a').click(function (){
...
function refrescarMultiplesCheckBox() {
    $(".check_todos").click(function (event) {
        ...
        if ($(this).is(":checked")) {
            $("#" + id + " .ck:checkbox:not(:checked)").prop("checked", "checked");
        } else {
...
function tr_label_for(){
    $(".tr_label_for").click(function (event){
        ...
        var selec_todos = $(this).closest("div.seleccionar_checks").find('.seleccionar_todos');
        if(selec_todos.is(':checked')){
            selec_todos.prop('checked',false);
        }
    });
...
function refrescarCheckTodos() {
    $(".seleccionar_todos").click(function (event) {
        var id = $(this).closest("div.seleccionar_checks").attr("id");
            //alert(id);

        if ($(this).is(":checked")) {
            $(this).closest('#'+id).find('input[type=checkbox]').prop('checked',true);
            //$("#" + id + " .ck:checkbox").attr("checked", "checked");
        } else {
            $(this).closest('#'+id).find('input[type=checkbox]').prop('checked',false);
            //$("#" + id + " .ck:checkbox").removeAttr("checked");
        }
    });
...
function refrescarCheckInvertir() {
    $(".invertir_seleccion").click(function (event) {
        var id = $(this).closest("div.seleccionar_checks").attr("id");
            //alert(id);

        $(this).closest('#'+id).find('input.ck').each(function(){
            if($(this).is(":checked")){
...
function buscarDocente(){
    ...
    $('#buscarDocente').chosen({
        no_results_text: "No encontrado...",
        width: "95%",
    }).change(function(){
        $("#buscarDocente option:selected").each(function () {
            docente_id = $(this).val();
            $.ajax({
                url: globalUrl + "resumen/revisionDocente/" + docente_id,
                success: function (datos) {
                    $("#contentDocente").html(datos);
                },
                complete: function () {
